...
class Poetry(object):
    ...
    def get_poetrys(self):
        ...
        for line in f.readlines():
            ...
            # 过滤含有特殊符号的唐诗
            if(not content or '_' in content or '(' in content or u'（' in content or u"□" in content
               or u'《' in content or '[' in content or ':' in content or u'：'in content):
                continue
            # 过滤较长或较短的唐诗
            if len(content) < 5 or len(content) > 79:
                continue
            content_list = content.replace(u'，', '|').replace(u'。', '|').split('|')
            flag = True
            # 过滤即非五言也非七验的唐诗
...
if __name__ == '__main__':
    ...
    print('valid poetry nums:', poetry.poetry_vectors_size)
